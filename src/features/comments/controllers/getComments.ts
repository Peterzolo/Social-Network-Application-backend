...
export class GetComment {
  public async comments(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'Post comments successfully fetched', comments });
  }
  ...
  public async commentsNamesFromCache(req: Request, res: Response): Promise<void> {
    ...
    res
      ...
      .json({ message: 'Post comments names successfully fetched', comments: commentsNames.length ? commentsNames[0] : [] });
  }
  ...
  public async singleComment(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'Single comment successfully fetched', comments: comments.length ? comments[0] : [] });
  }
}
